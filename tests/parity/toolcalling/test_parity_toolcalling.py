...
Run:
    pytest tests/parity/toolcalling/test_parity_toolcalling.py -v
"""
...
def _case_sort_key(case_id: str) -> tuple[int, str]:
    ...
    `TOOLCALLING.batch.5`   → (5, "")
    `TOOLCALLING.batch.5.d` → (5, "d")
    `TOOLCALLING.batch.8.a` → (8, "a")
    """
    parts = case_id.split(".")
    top = int(parts[2])
...
def _load_fixtures() -> list[tuple[str, str, str, dict[str, Any]]]:
    ...
    Two file layouts coexist:
      <family>/TOOLCALLING.<mode>.yaml       — legacy flat: holds 1, 2, ..., 10
      <family>/TOOLCALLING.<mode>.<n>.yaml   — per-top-level-case: holds n.a, n.b, ...

    `_run_parity_case` dispatches from the fixture-level `mode`, matching the
    `TOOLCALLING.batch.*` / `TOOLCALLING.stream.*` taxonomy directly.
    """
    out = []
    for fp in sorted(FIXTURES_ROOT.glob("*/TOOLCALLING.*.yaml")):
...
def _run_parity_case(
    ...
    parse_mod = importlib.import_module(f"tests.parity.toolcalling.{impl_name}")
    if mode == "stream":
