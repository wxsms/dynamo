...
Default — DRIFT CHECK (read-only):
    ...
    python3 -m tests.parity.toolcalling.capture_toolcalling_outputs --impl sglang --mode stream
...
`--merge` — POPULATE FIXTURES (destructive):
    ...
    python3 -m tests.parity.toolcalling.capture_toolcalling_outputs --impl vllm --merge
    python3 -m tests.parity.toolcalling.capture_toolcalling_outputs --impl sglang --merge
...
`--dump PATH` (optional, rarely needed):
    python3 -m tests.parity.toolcalling.capture_toolcalling_outputs --impl vllm --dump /tmp/v.json
...
def _case_sort_key(case_id: str) -> tuple[int, str]:
    """`TOOLCALLING.batch.8.a` → (8, 'a'); `TOOLCALLING.batch.5` → (5, '')."""
    parts = case_id.split(".")
