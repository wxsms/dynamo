...
In this operator-managed path, the EPP consumes live routing state through the Dynamo event plane using
NATS Core. Direct vLLM ZMQ KV-event subscriptions are used by other integration shapes, but not
...
With operator-managed GAIE, NATS Core backs live routing-state delivery. After a restart, the EPP
rebuilds prefix state from worker-local indexers and then consumes subsequent updates through the
Dynamo event plane.
