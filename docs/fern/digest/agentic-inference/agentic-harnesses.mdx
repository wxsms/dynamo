...
These changes build on the performance considerations outlined in our [first post](./agentic-inference.mdx), which focused on the serving architecture underneath agentic inference: the frontend, router, and KV cache management. This follow-up focuses on correctness, user-experience equivalence, and performance.
