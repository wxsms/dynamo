...
The Python and Rust pages are not committed: the docs pipeline generates them
at publish time, and the `Generated API References` pre-merge job reruns both
generators to prove a source change cannot break them. Only the Kubernetes
output stays committed, checked with `--check` scoped to the branch's own
changes on a pull request and strict on `main`.
</Note>
