...
Only `model` is unconditionally required by the schema. Admission rejects a new DGDR whose supplied `spec.image` has no parseable semantic-version tag unless `spec.runtimeVersionOverride` is set. When `spec.image` is omitted, the operator supplies a versioned default as described below.
...
<ParamField path="runtimeVersionOverride" type="string">
  Required when `spec.image` is set and has no parseable semantic-version tag, such as `latest`, a custom build tag, or a digest-only reference. Also supplies the default Dynamo runtime version for generated DGD components that do not set their own override. An explicit component value in `overrides.dgd` takes precedence. Set this when an effective generated component image has no parseable semantic-version tag or its tag does not identify the intended Dynamo runtime.

  Use canonical `MAJOR.MINOR.PATCH`, for example `"1.4.0"`, with each part between `0` and `9999` and no leading zeros, `v` prefix, prerelease suffix, or build metadata. This field does not change any image. See [Profiler image version compatibility](#profiler-image-version-compatibility).
</ParamField>
...
follows the [component runtime version rules](dynamo-component-deployment.mdx#runtime-version-compatibility) independently. An overridden image without a
