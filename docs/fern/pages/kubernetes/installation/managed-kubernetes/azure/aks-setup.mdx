...
<Steps toc={true}>
<Step title="Create a Resource Group and Cluster" id="step-1-create-a-resource-group-and-cluster">
```bash
az group create \
  --name <RESOURCE_GROUP> \
  --location <REGION>
...
<Step title="Add a GPU Node Pool" id="step-2-add-a-gpu-node-pool">
...
For RDMA-capable workloads (disaggregated inference), use ND-series VMs such as `Standard_ND96asr_v4` or `Standard_ND96isr_H100_v5`. See the [RDMA / InfiniBand guide](../../rdma-setup/infiniband-on-azure.mdx) for the additional setup required on those nodes.
...
<Step title="Install the NVIDIA GPU Operator" id="step-3-install-the-nvidia-gpu-operator">
...
<Note>
If you need RDMA / InfiniBand for disaggregated inference, **do not install the GPU Operator yet** — the RDMA setup requires different Helm values. See [RDMA / InfiniBand](../../rdma-setup/infiniband-on-azure.mdx) for the full setup, which includes the correct GPU Operator install command.
</Note>

</Step>
<Step title="Install Dynamo" id="step-4-install-dynamo">
Follow the [Installation Guide](../../install-dynamo.md) to install the Dynamo Platform and deploy your first model.
...
### [RDMA / InfiniBand](../../rdma-setup/infiniband-on-azure.mdx)
...
### [Storage for Model Caching](../../model-storage/aks-storage.md)
...
### [Azure Lustre CSI Driver](../../model-storage/azure-lustre-csi-driver.mdx)
