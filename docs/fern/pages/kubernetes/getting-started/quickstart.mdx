...
```
    ...
    For this DGDR quickstart, `DYNAMO_IMAGE` points to the Dynamo planner image. The planner profiles the model and creates the worker deployment; you do not need to choose a separate runtime image for the NVIDIA GPU path.
  </Step>
