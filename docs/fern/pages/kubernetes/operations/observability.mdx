...
`NIXL_TELEMETRY_ENABLE` on its own only turns collection on. A scrapeable endpoint also needs
`NIXL_TELEMETRY_EXPORTER=prometheus`, because NIXL has no default exporter.
`NIXL_TELEMETRY_PROMETHEUS_PORT` is optional and chooses the port. When it is unset, the exporter
falls back to `9090` rather than to Dynamo's `19090` convention. The operator already presets
`DYN_SYSTEM_PORT` to `9090`, so set the NIXL port explicitly to keep the two apart. The operator
presets `NIXL_TELEMETRY_EXPORTER=prometheus` and `NIXL_TELEMETRY_PROMETHEUS_PORT=19090` on worker,
prefill, and decode pods, so on an operator-managed deployment the snippet above is enough. Set all
three variables explicitly anywhere else.

A worker that runs several ranks in one container builds one NIXL agent per rank, so the configured
port is the base of a range: SGLang gives node-local rank *i* the port `base + i` and the operator
declares each one as a separate container port (`nixl`, `nixl-1`, …) when
`NIXL_TELEMETRY_ENABLE=y` (case-insensitive).

For upgrade compatibility, literal enable values `1`, `yes`, `on`, `true`, and `enable`
retain their existing container port declarations. To expose the additional SGLang rank ports,
change the value to `y` in the custom resource. This explicit change updates the pod template
and rolls the workload; an operator-only upgrade leaves these declarations unchanged.

<Note>
Enabling telemetry does not on its own make the port scrapeable from Prometheus. The
`dynamo-worker` PodMonitor endpoint for the `nixl` port is off by default
(`dynamo-operator.dynamo.metrics.podMonitors.nixlTelemetry` in the platform chart), because every worker pod declares the `nixl`
container port whether or not telemetry is enabled. Turn the endpoint on once the port is
confirmed to be serving.
</Note>
