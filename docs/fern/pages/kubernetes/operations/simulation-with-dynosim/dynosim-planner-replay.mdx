...
local deployment. Although `python -m dynamo.replay` executes on your local machine, this
tutorial evaluates Planner decisions for a Kubernetes deployment through the virtual simulation
environment.
</Note>

For the general replay workflow, see [Run a DynoSim Simulation](../../../cli/operations/simulation-with-dynosim/dynosim-replay.mdx). For Planner field types and
defaults, see the
