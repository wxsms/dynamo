...
title: Descriptor
subtitle: Registers a local memory buffer with the NIXL I/O subsystem for transfer.
---
...
<ParamField path="connector" type="Connector" required={true}>
  The [`Connector`](connector.mdx) whose NIXL subsystem the descriptor's memory buffer is registered with.
</ParamField>
...
<ParamField path="device" type="Device">
  A reference to the [`Device`](device.mdx) that contains the buffer the descriptor represents.
</ParamField>
