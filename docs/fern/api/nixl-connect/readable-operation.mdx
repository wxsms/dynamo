...
subtitle: Exposes local buffers for a remote worker to read via a paired ReadOperation.
---

An operation which enables a remote worker to read data from the local worker.

To create the operation, a set of local [`Descriptor`](descriptor.mdx) objects must be provided that reference memory intended to be transferred to a remote worker.
...
Generates and returns the NIXL metadata ([RdmaMetadata](rdma-metadata.mdx)) required for a remote worker to read from the operation.
Once acquired, the metadata needs to be provided to a remote worker via a secondary channel, most likely HTTP or TCP+NATS.
