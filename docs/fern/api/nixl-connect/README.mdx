...
for how they coordinate directly with the Encode Worker by creating a [`ReadableOperation`](readable-operation.mdx),
...
a [`ReadOperation`](read-operation.mdx) is created using the metadata provided by the requesting worker,
