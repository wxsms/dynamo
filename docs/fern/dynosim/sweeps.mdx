...
local CLI deployment. Use a sweep after a single [DynoSim run](runs.mdx) works and you want to identify
...
1. Run the configuration again with [Run a DynoSim Simulation](runs.mdx).
2. Exercise the live frontend and router with [Simulate a Local Deployment](mocker-local.mdx) or
   [Simulate a Kubernetes Deployment](mocker.mdx).
3. Deploy the candidate on the target hardware and benchmark it with AIPerf using either the
   [Kubernetes workflow](../tools/aiperf.mdx) or the [local CLI workflow](../cli/aiperf.mdx).
</Step>
</Steps>
