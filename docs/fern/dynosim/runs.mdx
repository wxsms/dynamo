...
Use [Simulate a Local Deployment with Mocker](mocker-local.mdx) instead when you need to exercise the
...
[Sweep DynoSim Configurations](sweeps.mdx) to search more worker and topology combinations.
...
[Kubernetes deployment](../tools/aiperf.mdx) or a [local CLI deployment](../cli/aiperf.mdx) to include
