...
registration, or runtime services, see [Run a DynoSim Simulation](runs.mdx).

This tutorial uses file-based discovery so that the frontend and worker can run without etcd or
NATS. For Kubernetes, see [Simulate a Kubernetes Deployment](mocker.mdx). For every available flag,
...
KV-cache, timing, AIC, and transport settings. Use [Run a DynoSim Simulation](runs.mdx) when you want
