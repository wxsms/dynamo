...
This is an architecture reference. For how to enable and tune migration, its benefits, and its known limitations, see the [Request Migration](../fault-tolerance/request-migration.md) use-case guide. For the metrics migration emits, see the [Metrics Catalog](../reference/observability/metrics-catalog.mdx#migration).
...
The migration limit and max sequence length are configured at the **frontend** level and apply globally to all models served by that frontend. See [Enable migration on the Frontend](../fault-tolerance/request-migration.md#enable-migration-on-the-frontend) in the use-case guide for the flags and environment variables.
...
- [Request Migration](../fault-tolerance/request-migration.md) - How to enable, tune, and reason about migration (use-case guide)
- [Request Cancellation Architecture](request-cancellation.md) - Canceling in-flight requests
- [Distributed Runtime](distributed-runtime.md) - Service discovery architecture that underpins failure detection
