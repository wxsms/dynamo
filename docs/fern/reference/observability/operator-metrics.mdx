...
<Note>
  These fields also appear in the CRD and Helm references. They are reproduced here so the observability enablement path is documented in one place. For the operator's full Helm value set, see the [Dynamo Operator guide](../../kubernetes/dynamo-operator.md).
</Note>
...
- [Kubernetes observability guide](../../kubernetes/observability/metrics.mdx) — signal enablement and the operator Grafana dashboard.
- [Kubernetes Metrics guide](../../kubernetes/observability/metrics.mdx) — PodMonitor and Prometheus walkthrough for application metrics.
