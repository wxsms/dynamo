...
For all KVBM settings, see the [KVBM Configuration Reference](../components/kvbm/kvbm-config-reference.mdx).
...
tier-aware shared-cache routing, see [SGLang HiCache](../backends/sglang/sglang-hicache.md). That
...
- Add [KV-aware routing](kv-routing.mdx) when multiple workers can reuse cached prefixes.
- Combine one offloading backend with [disaggregated serving](disaggregated-serving.mdx). In that
  ...
  decode workers.
- Review [KVBM architecture](../design-docs/kvbm-design.md) for Dynamo's cache-tier and transfer
  design.
- To make a custom inference engine visible to the KV router, implement
  [KV event publishing](../integrations/kv-events-custom-engines.md).
