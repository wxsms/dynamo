...
This tutorial adapts the aggregated vLLM deployment from [Deploy with DGD](dgd-guide.md) to run on
...
Complete the base [Deploy with DGD](dgd-guide.md) tutorial first, or be familiar with its Frontend,
...
matching registry credentials in the namespace, as described in [Deploy with DGD](dgd-guide.md#choose-your-backend).
...
See [Disaggregated Serving](../features/disaggregated-serving/README.md) for the DGD topology and
[Disaggregated Communication](disagg-communication-guide.md) for the network requirements. Validate
