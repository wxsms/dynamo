...
required. Use [KV-Aware Routing with the Dynamo Frontend](dgd-kv-routing.md) to enable KV-aware worker
selection in a DGD.
...
[Install Gateway API Routing](gateway-api-installation.mdx), then
...
For the cost model, see [Routing Concepts](../components/router/router-concepts.md). For shared tuning
guidance, see [Configuration and Tuning](../components/router/router-configuration.md).
