...
<ParamField path="experimental" type="DynamoGraphDeploymentExperimentalSpec">
  Graph-level opt-in preview features whose API shape may change in breaking ways between `v1beta1` releases. Component-level experimental features live under `spec.components[*].experimental` instead — see [DCD Reference — ExperimentalSpec](dcd-reference.mdx#experimentalspec).
</ParamField>
