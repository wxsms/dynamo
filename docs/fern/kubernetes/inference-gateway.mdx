...
title: Route Requests with Gateway API
sidebar-title: Gateway API
...
For direct-to-Frontend routing, use [KV-Aware Routing with the Dynamo Frontend](dgd-kv-routing.md).
...
- Gateway API, GAIE, and a compatible Gateway implementation installed. See
  [Install Gateway API Routing](gateway-api-installation.mdx).
...
- If the Gateway returns HTTP 500 in an Istio-injected namespace, verify that the Gateway proxy does
  ...
  [agentgateway and Istio injection](../components/router/gateway-api-reference.mdx#agentgateway-and-istio-injection).
