...
<ParamField path="eppConfig" type="EPPConfig">
  EPP-specific configuration. Only valid when `type` is `epp`. Exactly one of `configMapRef` or `config` must be set. See the [Gateway API Routing Reference](../components/router/gateway-api-reference.mdx).
</ParamField>
...
<ParamField path="status.component" type="ComponentReplicaStatus">
  Replica status for this component: desired, ready, and available counts. Shares the shape documented on the [DGD Reference](dgd-reference.mdx#componentreplicastatus).
</ParamField>
...
<ParamField path="checkpoint" type="ComponentCheckpointConfig">
  Configures container-image snapshotting and restore for the component. Set `checkpoint.enabled: true` to opt in; omit `checkpointRef` for a DGD-managed automatic checkpoint, or set it to restore an existing [DynamoCheckpoint](snapshot.md).
</ParamField>
